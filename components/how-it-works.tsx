export default function HowItWorks() {
  ...
  return (
    <div className="py-20 px-4 md:px-8 lg:px-20 max-w-[1440px] mx-auto">
      <h2 className="text-center text-[40px] md:text-[48px] font-violet-sans mb-24">
        ...
        {steps.map((step, index) => (
          ...
          >
            ...
            <div className="w-[120px] h-[120px] bg-white rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.08)] flex items-center justify-center mb-6 z-10">
              <span className="text-[36px] font-violet-sans text-[#6CB6FF]">
                {step.number}
              </span>
            </div>

            {/* Title */}
            <h3 className="text-[28px] font-violet-sans text-[#2e3b7d] mb-4">
              {step.title}
            </h3>

            {/* Description */}
            <p className="text-[#2e3b7d] max-w-[250px] leading-relaxed">
              {step.description}
            </p>
          </div>
        ))}
